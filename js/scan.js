...
function drawScan() {
	...
	for (var i = 0; i < scanJson.aps.length; i++) {
		...
		html += (selected ? "<tr class='selected'>" : "<tr>")
			...
			+ "<td class='lock'>" + (scanJson.aps[i][4] == "-" ? "" : lockico) + "</td>" // Lock Emoji
			+ "<td class='mac'>" + esc(scanJson.aps[i][5]) + "</td>" // MAC
			+ "<td class='vendor'>" + esc(scanJson.aps[i][6]) + "</td>" // Vendor
			// Select
			+ "<td class='selectColumn'><label class='checkbox-container'><input type='checkbox' class='ssidinp m5'" + (selected ? "checked" : "") + " onclick='selectRow(0," + i + "," + (selected ? "false" : "true") + ")'><span class='checkmark'></span></label></td>"
			+ "<td class='remove'><button class='attackpbtn butnstyle' onclick='remove(0," + i + ")'>X</button></td>" // Remove
			+ "</tr>";
	}
	...
	for (var i = 0; i < scanJson.stations.length; i++) {
		...
		html += (selected ? "<tr class='selected'>" : "<tr>")
			+ "<td class='id'>" + i + "</td>" // ID
			+ "<td class='vendor ptd'>" + esc(scanJson.stations[i][3]) + "</td>" 
			+ "<td class='mac'>" + esc(scanJson.stations[i][0]) + "</td>" 
			+ "<td class='ch'>" + esc(scanJson.stations[i][1]) + "</td>"
			+ "<td class='name ptd'>" + (scanJson.stations[i][2].length > 0 ? esc(scanJson.stations[i][2]) : "<button onclick='add(1," + i + ")'>" + lang("add") + "</button>") + "</td>"
			+ "<td class='pkts'>" + esc(scanJson.stations[i][4]) + "</td>"
			+ "<td class='ap ptd'>" + ap + "</td>"
			+ "<td class='lastseen'>" + esc(scanJson.stations[i][6]) + "</td>"
			+ "<td class='selectColumn'><label class='checkbox-container'><input type='checkbox' class='ssidinp m5'" + (selected ? "checked" : "") + " onclick='selectRow(1," + i + "," + (selected ? "false" : "true") + ")'><span class='checkmark'></span></label></td>"
			+ "<td class='remove'><button class='attackpbtn butnstyle' onclick='remove(1," + i + ")'>X</button></td>"
			+ "</tr>";
	}
...
function drawNames() {
	...
	for (var i = 0; i < nameJson.length; i++) {
		...
		html += (selected ? "<tr class='selected'>" : "<tr>")
			...
			+ "<td class='remove'><button class='attackpbtn butnstyle' onclick='remove(2," + i + ")'>X</button></td>" // Remove
			+ "</tr>";
	}
...
function scan(type) {
	switch (type) {
		case 0:
			...
			getE('scanZero').classList.add("blink");
			getE('scanZero').textContent = 'Scaning'
			elxtime = 2450;
			break;
		case 1:
			getE('scanZero').disabled = true;
			getE('scanOne').classList.add("blink");
			getE('scanOne').textContent = 'Scaning'
...
function buttonFunc() {
	const z = getE('scanZero'), o = getE('scanOne');
	if (duts === 0) {
		z.classList.remove('blink');
		o.disabled = false;
		z.textContent = 'Scan APs';
	} else {
		o.classList.remove('blink');
		o.textContent = 'Scan Station';
		z.disabled = false;
	}
